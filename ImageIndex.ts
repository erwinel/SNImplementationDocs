...
namespace imageIndex {
    ...
    interface ISvgSpriteInfo extends IImageDimensions {
        id: string;
    }
    interface IImageInfo extends IImageDimensions {
        title: string;
        fileName: string;
        sprites: ISvgSpriteInfo[];
    }

    interface IImageFolder {
        name: string;
        folders: IImageFolder[];
        images: IImageInfo[]
    }

    const rootFolder: IImageFolder = {
        "name": "images", "folders": [
            {
    ...
    function toThumbnailDimensions(width: number, height: number): IImageDimensions {
    ...
    interface IImageDataLink {
        name: string;
        selectFolder: { (event: BaseJQueryEventObject): void; }
    }
    ...
    interface IImageViewInfo extends IImageDimensions {
    ...
    function importImageInfo(imageArray: IImageInfo[], relativeUrl: string): IImageViewInfo[] {
        let resultArray: IImageViewInfo[] = [];
        let dim: IImageDimensions;
        imageArray.forEach((imageInfo: IImageInfo) => {
            let url: string = relativeUrl + "/" + encodeURIComponent(imageInfo.fileName);
            if (imageInfo.sprites.length > 0) {
                ...
                let isSpriteList: boolean = imageInfo.sprites.length > 1;
                resultArray.push({
                ...
                if (isSpriteList) {
                    for (let i: number = 1; i < imageInfo.sprites.length; i++) {
    ...
    interface IImageIndexScope extends ng.IScope {
        imageListController: ImageIndexController;
        folderName: string;
        breadcrumb: IImageDataLink[];
        subFolders: IImageDataLink[];
        images: IImageViewInfo[];
        hasFolders: boolean;
        hasImages: boolean;
        isTopLevel: boolean;
    }

    function preventDefault(event: BaseJQueryEventObject): void {
        if (sys.isNil(event))
            return;
    ...
    class ImageIndexController implements ng.IController {
        private _data: IImageData[] = [];

        constructor(private $scope: IImageIndexScope) {
            rootFolder.folders.forEach((f: IImageFolder) => { this.importImageData(f, '.'); });
            $scope.imageListController = this;

        }

        toImageDataLink(folder: IImageData): IImageDataLink {
            return {
                name: folder.name,
                selectFolder: (event: BaseJQueryEventObject) => {
        ...
        getSubFolderLinks(id?: number): IImageDataLink[] {
            return this._data.filter((d: IImageData) => d.parentId === id).map((d: IImageData) => { return this.toImageDataLink(d) }, this);
        }

        selectFolder(id?: number): void {
            let folder: IImageData | undefined = this.getImageData(id);
            ...
            if (this.$scope.isTopLevel) {
                ...
                let controller: ImageIndexController = this;
                this.$scope.breadcrumb = [{
        ...
        private getImageData(id?: number): IImageData {
            if (typeof id === "number") {
        ...
        private importImageData(folder: IImageFolder, relativeUrl: string, parent?: IImageData): void {
            relativeUrl += "/" + folder.name;
            let id: number = this._data.length;
            let current: IImageData = {
