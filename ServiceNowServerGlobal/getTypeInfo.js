...
function getElementResult(element) {
    var result;
    try { result = { 'object': element, inherits: getInheritance(element) }; }
    catch (e) { result = { source: element }; }
    var glideObj;
    var nObj;
    try {
        if (element.getGlideObject) {
            glideObj = element.getGlideObject();
            try { nObj = { 'object': glideObj, inherits: getInheritance(glideObj) }; }
            catch (e) { nObj = { source: glideObj }; }
            result = {
                glide: nObj,
                source: result
            };
...
function getInfo(colName, isArray, array_denormalized) {
  ...
  var colName = 'sys_class_name';
  
  var singular = getInfo(colName, false, false);
  var asArray = getInfo(colName, true, false);
  var arrayDenormalized = getInfo(colName, false, true);
  var result;
  if (typeof singular !== 'undefined') {
      if (typeof asArray !== 'undefined') {
          ...
          if (typeof arrayDenormalized !== 'undefined')
              result.arrayDenormalized = arrayDenormalized;
      } else if (typeof arrayDenormalized !== 'undefined')
          result = {
              singular: singular,
              arrayDenormalized: arrayDenormalized
          };
      else
          result = singular;
  } else if (typeof asArray !== 'undefined') {
      if (typeof arrayDenormalized !== 'undefined')
          result = {
              arrayDenormalized: arrayDenormalized,
              asArray: asArray
