...
namespace inicidentManagment {
    interface ISelectionOption { value: number, text: string; }
    interface IFieldExplanation { class: string, colSpan: number; }
    interface IUsersImpactedVar {
        label: string;
        selected: number
        options: ISelectionOption[];
        isAutoValue: boolean,
        valueClass: string,
        explanation: IFieldExplanation;
    }
    interface IProductivityImpactedVar {
        label: string;
        selected: number;
        options: ISelectionOption[];
        isAutoValue: boolean,
    ...
    interface IIsMissionRelatedVar {
        label: string;
        value: YesNoValue;
        operand: number;
        explanation: IFieldExplanation;
    }
    interface ICallerVipVar {
        label: string;
        value: boolean;
        operand: number;
    };
    interface IImpactVar {

    }
    interface IUrgencyVar {
    ...
    interface IIncidentManagmentScope extends ng.IScope {
        users_impacted: IUsersImpactedVar;
        productivity_impact: IProductivityImpactedVar;
        is_mission_related: IIsMissionRelatedVar;
        caller_vip: ICallerVipVar;
        impact: IImpactVar;
        urgency: IUrgencyVar;
        comment: string;
    }
    ...
    class IncidentManagmentController implements ng.IController {
        ...
        $doCheck() { }
        emulateSubmitAnIncidentBusinessRule(producer: IEmulatedProducer, current: sn.GlideRecord, caller): void {
