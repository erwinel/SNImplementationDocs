...
namespace Incident_caller_profile_validation {
    declare var current: IincidentGlideRecord;
    declare var previous: IincidentGlideRecord | undefined;
    (function executeRule(current: IincidentGlideRecord, previous?: IincidentGlideRecord /*null when async*/) {
        ...
        var sys_id: string = current.caller_id.sys_id.getValue();
        if (sys_id === gs.getUserID())
            ...
            if (gs.hasRole('user_admin,itil,admin'))
                ...
                gs.addErrorMessage(message);
            gs.eventQueue('army.incident_caller.profile_incomplete', <incidentGlideRecord>current, sys_id);
        }
    })(current, previous);
}